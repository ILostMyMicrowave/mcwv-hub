  ...
  try {
    ...
    if (!exists.rows[0]?.exists) {
      return NextResponse.json(
        { success: true, event: null, entrants: [], feed: [], rounds: [], standings: [], updatedAt: new Date().toISOString() },
    ...
    return NextResponse.json({ success: false, error: "Failed to load bounty state" }, { status: 500 });
  }
}
