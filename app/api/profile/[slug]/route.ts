...
function choosePetIcon(pet: Record<string, any>, fallbackName: string, golden: boolean) {
  const directIcon =
    typeof pet.icon === "string" && pet.icon.trim() !== "" ? String(pet.icon) : "";
  const directGoldenIcon =
    typeof pet.goldenIcon === "string" && pet.goldenIcon.trim() !== ""
      ? String(pet.goldenIcon)
      : "";
