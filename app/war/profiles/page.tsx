...
type Tab = "roster" | "gems" | "showcase" | "gamepass" | "improved" | "timeline";

const TABS: { id: Tab; label: string; icon: string }[] = [
  { id: "roster", label: "Roster", icon: "👥" },
  { id: "gems", label: "Gem Leaderboard", icon: "💎" },
  { id: "showcase", label: "Pet Showcase", icon: "🐾" },
...
export default function WarProfilesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter(
        (m) => m.username.toLowerCase().includes(q) || m.robloxId.includes(q)
      );
    }
    ...
    if (masteryFilter !== null && masteryFilter > 0) {
      list = list.filter(
        (m) => m.connected && m.masteryAverage !== null && m.masteryAverage < masteryFilter
      );
    }
    return list;
  ...
  const gemBoard = useMemo(
    () =>
      members
        .filter((m) => m.connected && m.gems !== null)
        .sort((a, b) => (b.gems ?? 0) - (a.gems ?? 0)),
    [members]
  );

  // Most improved: members with a positive gem delta over the lookback window,
  // sorted by delta descending (most gained first).
  const improved = useMemo(
    () =>
      members
        .filter((m) => m.connected && m.gemDelta !== null)
        .sort((a, b) => (b.gemDelta ?? 0) - (a.gemDelta ?? 0)),
    [members]
  );

  // Gamepass coverage across connected members.
  const gamepassStats = useMemo(() => {
    const counts = new Map<string, number>();
    for (const m of connected) {
      for (const g of m.gamepasses) counts.set(g, (counts.get(g) || 0) + 1);
    }
    ...
    <main className="min-h-screen bg-[#070b1a] px-3 py-5 text-zinc-200 sm:px-5">
      <div className="mx-auto max-w-6xl">
        <header className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-400">
              {members.length} members · {connected.length} connected
            </p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Card label="Avg Rank Stars" value={avgRankStars === null ? "—" : String(avgRankStars)} />
        ...
        {tab === "gems" && (
          <GemTab members={gemBoard} onSelect={setSelected} />
        )}

        {tab === "showcase" && (
          <ShowcaseTab members={connected} selected={selectedMember} onSelect={setSelected} />
        )}

        {tab === "gamepass" && (
          <GamepassTab stats={gamepassStats} members={connected} />
        )}

        {tab === "improved" && (
          <ImprovedTab members={improved} />
        )}

        {tab === "timeline" && (
          <TimelineTab points={warTimeline} />
        )}
      </div>
    </main>
  );
    ...
    <div className="mt-4">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div className="flex items-center gap-1">
          <Select
            ...
            options={[
              ["none", "Gems"],
              ...GEM_PRESETS.map((p) => [String(p.value), p.label] as [string, string]),
            ]}
          />
        </div>
        <input
      ...
      <div className="mt-3 overflow-x-auto rounded-2xl border border-white/10 bg-black/20">
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            {props.filtered.map((m) => (
              <tr key={m.robloxId} className="border-b border-white/5 hover:bg-white/5">
                <td className="px-3 py-2">
                  <Link href={`/profile/${m.username}`} className="font-semibold text-violet-200 hover:underline">
                    {m.username}
                  </Link>
                </td>
                <td className="px-3 py-2 capitalize text-zinc-300">{m.role}</td>
                <td className={`px-3 py-2 ${belowThreshold(m.gems, props.avgGems) ? "font-bold text-red-400" : "text-zinc-200"}`}>
                ...
                <td className="px-3 py-2">
                  {m.connected ? (
                    ...
                    <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-xs font-semibold text-amber-300">⚠️ Not connected</span>
                  )}
                </td>
              </tr>
            ))}
        ...
        {!props.filtered.length && (
          <div className="px-4 py-10 text-center text-sm text-zinc-500">No members match these filters.</div>
        )}
      </div>
    </div>
  );
...
function belowThreshold(gems: number | null, avgGems: number | null): boolean {
  ...
  return gems < avgGems * 0.5; // below half the average = flagged
}
...
function Select(props: { value: string; onChange: (v: string) => void; options: [string, string][] }) {
  return (
    <select
      ...
      className="w-full rounded-xl border border-white/10 bg-[#0c1130] px-2 py-2 text-sm text-white outline-none focus:border-violet-500"
    >
      {props.options.map(([v, l]) => (
        <option key={v} value={v} className="bg-[#0c1130]">{l}</option>
      ))}
    </select>
  );
}
  ...
  return (
    <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
      {members.map((m, i) => (
        <button
          key={m.robloxId}
          onClick={() => onSelect(m.robloxId)}
          className="flex items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-left hover:bg-white/10"
        >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {members.map((m) => (
          <button
            key={m.robloxId}
            onClick={() => onSelect(m.robloxId)}
            className={`whitespace-nowrap rounded-xl px-3 py-1.5 text-sm ${selected?.robloxId === m.robloxId ? "bg-violet-600 text-white" : "bg-white/5 text-zinc-300 hover:bg-white/10"}`}
          >
            {m.username}
          </button>
        ))}
      ...
      {selected ? (
        <div className="mt-3 rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="flex items-center gap-3">
            ...
            <img src={selected.avatarUrl ?? ""} alt="" className="h-12 w-12 rounded-full" />
            <div>
              <Link href={`/profile/${selected.username}`} className="text-lg font-bold text-white hover:underline">
                {selected.username}
              </Link>
              <div className="text-xs text-zinc-400">{fmt(selected.gems)} gems · Mastery {fmt(selected.masteryAverage)}</div>
            </div>
          </div>
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {selected.equippedPets.length ? (
                selected.equippedPets.map((p, i) => (
        ...
        <div className="mt-3 rounded-2xl border border-dashed border-white/10 py-10 text-center text-sm text-zinc-500">
          Select a connected member to see their showcase.
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20">
      <div className="border-b border-white/10 px-4 py-2.5 text-xs text-zinc-400">
        Gem change over the last 14 days — most gained first
      </div>
      <div className="max-h-[60vh] divide-y divide-white/5 overflow-y-auto">
        ...
        {!members.length && (
          <div className="py-10 text-center text-sm text-zinc-500">
            No gem snapshots yet — they accumulate as the Profiles page is viewed.
          </div>
        )}
      </div>
    </div>
...
function TimelineTab({ points }: { points: WarTimelinePoint[] }) {
  ...
  if (!data.length) {
    return (
      <div className="mt-4 rounded-2xl border border-dashed border-white/10 py-14 text-center text-sm text-zinc-500">
        No war timeline snapshots yet.
      </div>
    );
  }
        ...
        {data.map((p, i) => {
          ...
          return (
            <div key={i} className="group relative flex-1" title={`${timeLabel} · ${fmt(p.points)} pts · #${p.rank ?? "?"}`}>
              <div className="absolute -top-6 hidden whitespace-nowrap rounded bg-zinc-800 px-1.5 py-0.5 text-[10px] group-hover:block">
                {fmt(p.points)} · #{p.rank ?? "?"}
              </div>
