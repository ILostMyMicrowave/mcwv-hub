...
return <div key={lineIndex} className="h-2" />;
    ...
    while ((match = regex.exec(line)) !== null) {
      ...
      if (token.startsWith("**") && token.endsWith("**")) {
        nodes.push(
          <strong key={`b-${lineIndex}-${key++}`} className="font-semibold text-white">
            {token.slice(2, -2)}
          </strong>
        );
      } else if (token.startsWith("__") && token.endsWith("__")) {
        nodes.push(
          <u key={`u-${lineIndex}-${key++}`} className="underline-offset-2">
            {token.slice(2, -2)}
          </u>
        );
      } else if (token.startsWith("*") && token.endsWith("*")) {
        nodes.push(
          <em key={`i-${lineIndex}-${key++}`} className="italic text-white/95">
            {token.slice(1, -1)}
          </em>
        );
      }
